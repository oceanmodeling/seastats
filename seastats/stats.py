...
def get_bias(sim: pd.Series, obs: pd.Series) -> float:
    return float(sim.mean() - obs.mean())


def get_mse(sim: pd.Series, obs: pd.Series) -> float:
    return float(np.square(np.subtract(obs, sim)).mean())


def get_rmse(sim: pd.Series, obs: pd.Series) -> float:
    return float(np.sqrt(get_mse(sim, obs)))


def get_mae(sim: pd.Series, obs: pd.Series) -> float:
    return float(np.abs(np.subtract(obs, sim)).mean())


def get_mad(sim: pd.Series, obs: pd.Series) -> float:
    return float(np.abs(np.subtract(obs, sim)).std())


def get_madp(sim: pd.Series, obs: pd.Series) -> float:
    pc1, pc2 = get_percentiles(sim, obs)
    return get_mad(pc1, pc2)


def get_madc(sim: pd.Series, obs: pd.Series) -> float:
    madp = get_madp(sim, obs)
    return get_mad(sim, obs) + madp


def get_rms(sim: pd.Series, obs: pd.Series) -> float:
    crmsd = ((sim - sim.mean()) - (obs - obs.mean())) ** 2
    return float(np.sqrt(crmsd.mean()))


def get_corr(sim: pd.Series, obs: pd.Series) -> float:
    return float(sim.corr(obs))


def get_nse(sim: pd.Series, obs: pd.Series) -> float:
...
def get_lambda(sim: pd.Series, obs: pd.Series) -> float:
...
def get_kge(sim: pd.Series, obs: pd.Series) -> float:
    corr = get_corr(sim, obs)
...
def get_slope_intercept(sim: pd.Series, obs: pd.Series) -> tuple[float, float]:
...
def get_slope_intercept_pp(sim: pd.Series, obs: pd.Series) -> tuple[float, float]:
    pc1, pc2 = get_percentiles(sim, obs)
